(function ($) {
  if ("undefined" == typeof jQuery)
    throw new Error(
      "Best QSystems  Framework requires JQuery to run. Please install any version ~ Benson Karue "
    );
...
function openDialog() {
  ...
  if (dim > 1038) {
    ...
    $(".band-connection").css("left", "30%");
    $(".system-title").html(
      "Murang'a University Classes Management Stystem (MCMS)"
    );
    $(".right").css("right", "24%");
  }
  ...
  $("body").on("click", ".notification-manager", function (event) {
    ...
    function realTime() {
      ...
      $(".notification-manager .notification-content")
        .addClass("animated wobble infinite")
        .removeClass("slideInLeft");
      if (time == 1) {
        $(".notification-manager .notification-content").removeClass(
          "wobble infinite"
        );
        clearTimeout(id);
      }
...
function openAllDialogTypes() {
  ...
  function realTime() {
    ...
    if ((time = 1)) {
      if (
        $("[data-show='dialog']").is(":visible") ||
        $("[data-show='mobile-menu']").is(":visible") ||
        $("[data-show='side-bar']").is(":visible")
      ) {
        $("body").addClass("open-customs");
      } else {
        $("body").removeClass("open-customs");
